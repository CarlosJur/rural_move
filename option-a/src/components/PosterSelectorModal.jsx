  ...
  return (
    ...
    >
      ...
      <div className="px-8 py-5" style={{ backgroundColor: c.dot }}>
        <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(255,255,255,0.2)', color: 'white' }}>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1.5">
                    ...
                    {conductor && <span className="text-xs text-slate-500">👤 {conductor}</span>}
                    {a.lugar && <span className="text-xs text-slate-500">📍 {a.lugar}</span>}
                    {a.plazas != null && (
                      <span className="text-xs text-slate-500">👥 {a.participantes.length}/{a.plazas}</span>
                    )}
                  </div>
                </div>
              </div>
...
export default function PosterSelectorModal({ actividades, onClose }) {
  ...
  const [step, setStep] = useState('select') // 'select' | 'juntos' | 'separado'
            ...
            {actividades.map(a => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${c.bgLight} ${c.text} flex-shrink-0`}>
                    {c.icon} {c.label}
                  </span>
      ...
      <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl my-4">
          ...
          <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-200">
            ...
            >
              {downloading ? 'Generando…' : '⬇ Descargar cartel'}
            </button>
          </div>
        </div>
      </div>
